...
public class TopTermsTest
{
    ...
    private static final File output = new File("src/test/resources/test_storage/TopTermsTest/test_results.txt");
    private static final File outputFolder = new File("src/test/resources/test_storage/TopTermsTest");
    private static final File comparison = new File("src/test/resources/test_source/TopTermsTest/expected_results.txt");
    
    @Test
    void testtopterms() throws Exception
    {
        File[] input = inputFolder.listFiles((dir, name)->name.endsWith(".json"));//cerca file .json nella cartella folder
        if(input != null)
        {
            ...
            terms = (articles.getTerms().entrySet().stream().sorted((a, b) -> a.getKey().compareTo(b.getKey()))
            ...
            if(!alpha.equals(beta))//confronta il risultato di questo test con il risultato previsto
            {
                Assertions.fail("I risultati previsti sono diversi");
            }        
        }
        else Assertions.fail("Non c'è nessun file .json nella cartella src/test/resources/test_source/TopTermsTest");
